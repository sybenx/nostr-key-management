...
NKTS is a tiered system utilizing the most secure method of key management where available first: FROST, iCloud Storage, Google secure sync, secure gift-wrap over public relays, and is compatible with Bluetooth and LAN transfer. It is fully compatible with FROST and can silently upgrade with a compatible FROST server. When FROST is activated, there are protections from various angles of attack including single device compromise and single server compromise. Even before FROST is activated, there is strong shoulder surfing resistance. Users are encouraged to make themselves secure as much or as little as they prefer and where robust defaults can be chosen for the user, within reasonable constraints, they are. 
