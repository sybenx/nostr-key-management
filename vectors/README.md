...
| `tiers-profile-a.json` | TIERS §4.2, §4.4, §4.5 — Profile A's reference membership (`k = 4`), its constraint results and slacks, and all fourteen minimal signing sets in the table's order; and the `k = 6, T = 3` configuration §4.2 gives for (7) on with a grant. |
...
form one, (2) exactly when grants alone do, where (6) holds no minimal set is one
trusted device with grants, and (7) never holds where (6) does not. Party names are fixtures.
